<script lang="ts">
  ...
  let mailboxHref = $derived(data.email && data.hits?.[0]?.mailbox ? `/folder/${encodeURIComponent(data.hits[0].mailbox)}` : '/inbox');
</script>
...
<div class="flex h-full flex-col">
  <div class="border-b bg-card px-4 py-3 sm:px-6">
    <div class="flex items-center gap-3 mb-3">
      ...
      <h1 class="text-sm font-semibold flex items-center gap-2"><Search class="size-4 text-primary" /> Búsqueda</h1>
    </div>
    <form class="flex gap-2 items-end" onsubmit={runSearch}>
      ...
      <div class="space-y-1.5">
        ...
        <select id="scope" bind:value={scope} class="flex h-9 rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
  ...
  <div class="flex-1 overflow-y-auto">
    ...
    {:else}
      <p class="px-6 pt-4 text-xs text-muted-foreground">{data.hits.length} resultado(s) para "{data.query}"</p>
      <ul class="divide-y">
        {#each data.hits as m}
          <li class="group relative flex gap-3 px-4 py-3 transition-colors hover:bg-accent/40 sm:px-6" class:bg-accent={!m.seen}>
            <a href={`/thread/${m.uid}?mailbox=${encodeURIComponent(m.mailbox)}`} class="absolute inset-0 z-10" aria-label={m.subject}></a>
            {#if !m.seen}
              <span class="absolute left-1 top-1/2 -translate-y-1/2 size-1.5 rounded-full bg-primary"></span>
            {/if}
            <Avatar alt={m.from} fallback={senderName(m.from).slice(0, 2).toUpperCase()} class="size-9 mt-0.5 z-20 pointer-events-none border" />
