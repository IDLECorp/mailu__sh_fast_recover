<script lang="ts">
  ...
  import { KeyRound, Eye, EyeOff, AlertCircle, Loader2, ShieldAlert, ExternalLink } from 'lucide-svelte';
...
<main class="min-h-screen flex items-center justify-center px-4 py-10">
  <div class="w-full max-w-md space-y-4">
    <div class="flex flex-col items-center text-center space-y-2 mb-2">
      ...
      <h1 class="text-xl font-semibold tracking-tight">Cambiá tu contraseña</h1>
      <p class="text-sm text-muted-foreground px-6">
        Tu cuenta <span class="font-medium">{data.email}</span> tiene una contraseña temporal. Necesitás cambiarla antes de poder usar Fast Mail.
      </p>
    </div>
          ...
          <div class="flex items-start gap-3 text-sm">
            ...
            <div class="space-y-2">
              <p class="text-sm text-muted-foreground">
                La API de Mailu está apagada (<code class="text-xs bg-muted px-1 rounded">API=false</code>). Para cambiar password desde acá, hay que habilitarla.
              </p>
              <a href="https://mail.nexuscorpec.com/admin" target="_blank" rel="noopener" class="inline-flex items-center gap-1 text-primary text-sm hover:underline">
                Cambiar desde mail.nexuscorpec.com/admin <ExternalLink class="size-3" />
              </a>
            </div>
          </div>
        </CardContent>
    ...
    {:else}
      <Card>
        <CardHeader>
          <CardTitle class="text-lg">Nueva contraseña</CardTitle>
          <CardDescription>Mínimo 8 caracteres. No puede ser igual a la actual.</CardDescription>
        </CardHeader>
        <CardContent>
          <form
            method="POST"
          ...
          >
            <div class="space-y-2">
              ...
              <div class="relative">
                ...
                <Input id="current" name="current" type={showCurrent ? 'text' : 'password'} autocomplete="current-password" required class="pl-9 pr-9" placeholder="••••••••" />
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                <KeyRound class="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground pointer-events-none" />
                <Input id="new" name="new" type={showNew ? 'text' : 'password'} autocomplete="new-password" required minlength={8} class="pl-9 pr-9" placeholder="••••••••" />
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                <KeyRound class="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground pointer-events-none" />
                <Input id="confirm" name="confirm" type={showConfirm ? 'text' : 'password'} autocomplete="new-password" required minlength={8} class="pl-9 pr-9" placeholder="••••••••" />
